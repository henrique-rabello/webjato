...
angular.module("WebjatoServices").service("HelpIndexer", (ContentType) => {
    var folder: string = "/help/";
    var items: Array<HelpBit> = [
    ...
    return {
        GetUrl: function (id: string): string {
            return folder + _.find(items, (item: HelpBit): boolean => { return item.Id === id; }).Url;
        },
        GetIdByContentType: function (type): string {
            switch (type) {
